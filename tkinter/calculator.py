...
def do_plus():
    "Set first term & clear typed number, when clicked + button"
...
def do_eq():
    "Set second term, run addition & clear typed number, when clicked = button"
    global second_term
    global result
    global current_number
...
# Or b1 = tk.Button(f, text = "1", command = lambda:key(1))
